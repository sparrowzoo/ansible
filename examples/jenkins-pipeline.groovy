        ...
        stage('deploy'){
            steps {
                sh ''' 
					...
					cp ${WORKSPACE}/${project_name}/${service_name}/target/${service_name}-1.0-SNAPSHOT.jar  $backup_dir/
					
					deploy_ansible_root="/Users/bitmain/it/ansible/examples"

					inventory_host="sdevice"
					app_file="${backup_dir}/${service_name}-1.0-SNAPSHOT.jar"


                    if [ ! -e $app_file ];then echo "${app_file} does't exist!" exit -1;fi
                    app_file_dest_path="~/.jenkins/${service_name}.jar"

					ansible-playbook ${deploy_ansible_root}/deploy.yml \
					--extra-vars "app_file=$app_file app_file_dest_path=$app_file_dest_path inventory_host=$inventory_host"
				'''
            }
        }
    }
